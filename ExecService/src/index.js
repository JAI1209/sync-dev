      ...
      if (!filesLoaded) {
        try {
          ...
          if (msg.type === 'init') {
            ...
            const ptyProcess = pty.spawn('docker', ['exec', '-it', session.container.id, '/bin/sh'], { name: 'xterm-256color', cols: msg.cols || 120, rows: msg.rows || 30, env: { ...process.env, TERM: 'xterm-256color', HOME: '/workspace' } });
