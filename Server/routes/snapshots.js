...
router.post("/:roomId/restore", authJwt, requirePermission("RESTORE_SNAPSHOT"), async (req, res) => {
  ...
  try {
    ...
    // Bug C: Update live in-memory rooms state
    const { rooms } = require("../server");
    if (rooms[roomId]) {
      rooms[roomId].files = Object.fromEntries(snapshot.files);
      rooms[roomId].folders = Object.fromEntries(snapshot.folders);
      rooms[roomId].activeFile = snapshot.activeFile;
