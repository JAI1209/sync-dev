...
const CHECKS = {
  ...
  client: [
    ...
    { file: "Client/src/hooks/useSocket.js", check: /socket\.on\("bulk-imported"/, desc: "bulk-imported handler exists" },
    { file: "Client/src/pages/Editor.jsx", check: /userRole === "viewer" \? "[^\"]*View Only"/, desc: "Upload shows View Only for viewers" },
    { file: "Client/src/hooks/useSocket.js", check: /socket\.on\("role-changed"/, desc: "role-changed socket handler" },
  ],
};
...
for (const [section, checks] of Object.entries(CHECKS)) {
  ...
  for (const { file, check, desc } of checks) {
    ...
    if (result.status === "pass") {
      ...
      console.log(`  [MISS] ${desc} - FILE MISSING`);
      missing += 1;
    }
  }
...
if (failed > 0) {
  ...
  console.log("\nSome files missing but critical features present");
  process.exit(0);
} else {
